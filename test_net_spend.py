...
def build_gmail_query(start_date: str, end_date: str) -> str:
    """
    Gmail search query. Gmail's `before:` is exclusive of that day, so we
    bump end_date by one day to make the range inclusive.
    start_date / end_date format: YYYY-MM-DD
    """
    start_fmt = datetime.strptime(start_date, "%Y-%m-%d").strftime("%Y/%m/%d")
    end_dt = datetime.strptime(end_date, "%Y-%m-%d")
    end_fmt_exclusive = end_dt.replace(day=end_dt.day)
    # add one day for inclusive end
    from datetime import timedelta
    end_fmt_exclusive = (end_dt + timedelta(days=1)).strftime("%Y/%m/%d")

    query = (
        f'from:({SENDER_QUERY}) subject:("{SUBJECT_QUERY}") '
        f"after:{start_fmt} before:{end_fmt_exclusive}"
    )
    return query
...
def main():
    parser = argparse.ArgumentParser(description="Extract Axis Bank transaction alerts from Gmail into Excel.")
    parser.add_argument("--start", required=True, help="Start date YYYY-MM-DD")
    parser.add_argument("--end", required=True, help="End date YYYY-MM-DD (inclusive)")
    parser.add_argument("--out", default="axis_transactions.xlsx", help="Output Excel file path")
    args = parser.parse_args()

    transactions = fetch_axis_transactions(args.start, args.end)
    write_to_excel(transactions, args.out)
